...
export default function UploadSection({ selectedTool }: UploadSectionProps) {
  ...
  const runConversion = async () => {
    ...
    try {
      ...
      setState('converting');
      await animateProgress(30, 70, 1000);

      let result: { blob: Blob; filename: string };

      if (activeTool.to.toLowerCase() === 'pdf' && file.type.startsWith('image/')) {
        result = await convertImageToPdf(file);
      } else {
        throw new Error(`Conversion from ${file.type} to ${activeTool.to} is not supported.`);
      }
            ...
            {isProcessing && (
              ...
              >
                ...
                <p className="text-slate-500 text-sm mb-8">
                  {state === 'uploading' ? 'Preparing your file...' : 'Applying conversion...'}
                </p>
                ...
                <p className="text-slate-600 text-xs mt-3">
                  {file?.name} &bull; {file ? formatBytes(file.size) : ''}
                </p>
              </motion.div>
            )}
            ...
            {state === 'done' && (
              ...
              >
                ...
                <p className="text-slate-500 text-xs mb-8">
                  Original: {file ? formatBytes(file.size) : ''} {'->'} Converted: {outputSize ? formatBytes(outputSize) : ''}
                </p>
